...
public class ReportAbuseApi extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("doPost called on /reportabuse");
		...
		try (Transaction tx = BokwasDB.getDatabase().beginTx()) 
		{
			if (postId != null) 
			{
				...
				if (commentId == null)
				{
					if (post.getAbuseCount() < (threshold - 1))
					{
						if (!post.getReportedByUserIds().contains(personId)){
							post.setAbuseCount();
							post.setReportedByUserIds(personId);
						}
					}
					else
					{
				...
				{
					Comments comment = post.getComment(commentId);
					if (comment.getAbuseCount() < (threshold - 1))
					{
						if (!comment.getReportedByUserIds().contains(personId)){
							comment.setAbuseCount();
							comment.setReportedByUserIds(personId);
						}
					}
					else
					{
