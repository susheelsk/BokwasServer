...
public class SinglePostResponse {
	...
	public SinglePostResponse(HashMap<Posts, List<Comments>> map) {
		super();
		...
		for (Map.Entry<Posts, List<Comments>> entry : map.entrySet()) {
			...
			posts.add(new Post(friendPost.getPostId(), friendPost.getPostTime(), friendPost
					...
					.isBokwasPost(),friendPost.getPostedByName(),friendPost.getAvatarId(),friendPost.getPostUpdatedTime(),friendPost.getFbPicture(),postlikes, postCommentList));
		}
	...
	private class Post {
		...
		public Post(String post_id, long created_time, String message,
				String posted_by, boolean isBokwasPost,String bokwasname, String avatarid,long update_time,String pic_link, List<Like> likes, List<Comment> comments) {
			super();
